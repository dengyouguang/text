...
$this->title = '深层补水日夜保湿面膜 ';
?>
<div>
    <?= Html::a(Html::img("images/header.png",['width'=>'100%']),'http://www.dcexport.cn')?>
    <p style="font-size:1.5em;text-align: center;margin:1em auto">DC Export深层补水日夜保湿面膜</p>
    <p style="text-align: center;color:red">（官方活动：购买259元DC深层补水日夜保湿面膜，送129元48小时焕肤套装）</p>
</div>
<div id="img">
    <?php
        echo Html::img('images/product2.jpg',['width'=>'100%']);
        for ($i=2; $i<=24 ; $i++) {
            echo Html::img($i<10 ? "images/product_02_0".$i.'.jpg' : "images/product_02_".$i.'.jpg',['width'=>'100%','alt'=>"深层补水日夜保湿面膜"]);
        }
    ?>
</div>
<hr />
...
<div style="margin-top: 10px;">
    ...
    ?>
    <div style="margin:10px 5%">数量：<button type="button" id="cut">-</button><input id="activityorder-num" style="width: 10%;margin-left:0"  type="text" value="1" name="Activityorder[num]"><button type="button" id="add">+</button>  总价：<b>259</b>.00元</div>
    ...
    <?= $form->field($model, 'goods')->hiddenInput(['value'=>'深层补水日夜保湿面膜'])->label(false) ?>
    ...
    <?= $form->field($model, 'total', ['inputOptions' => ['placeholder' => '总价']])->hiddenInput(['value' => 259])->label(false) ?>
...
<div style="width:90%;margin: 0 auto;clear: both">
    ...
    <div class="shuxing" style="display: none">
        <table class="table">
            ...
            <tr>
                <td>[主要成份]</td><td>水、丁二醇、马齿苋（PORTULACA OLERACEA）提取物、甲基葡糖醇聚醚-20、1,2-戊二醇、透明质酸钠、泛醇、尿囊素、丙烯酸（酯）类/C10-30 烷醇丙烯酸酯交联聚合物、聚乙二醇-90M、羟苯甲酯、双（羟甲基）咪唑烷基脲、碘丙炔醇丁基氨甲酸酯、PEG-40 氢化蓖麻油、香精。</td>
            </tr>
            <tr>
                <td>[使用部位]</td><td>面部</td>
            </tr>
